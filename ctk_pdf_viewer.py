...
class CTkPDFViewer(customtkinter.CTkScrollableFrame):
    def __init__(
        ...
        **kwargs,
        ...
        self.loading_message = customtkinter.CTkLabel(
            self, textvariable=self.percentage_load, justify="center"
        )
    ...
    def add_pages(self):
        ...
        for page in open_pdf:
            ...
            label_img = customtkinter.CTkImage(
                img, size=(self.page_width, self.page_height)
            )
            ...
            percentage_view = (
                float(self.percentage_bar) / float(len(open_pdf)) * float(100)
            )
            self.loading_bar.set(percentage_view)
            basename = os.path.basename(self.file)
            percent_text = int(math.floor(percentage_view))
            self.percentage_load.set(f"Loading {basename} \n{percent_text}%")
