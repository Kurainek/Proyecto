...
class BoletaFacade:
    ...
    def generar_detalle_boleta(self):
        ...
        for item in self.pedido.menus:
            if item.cantidad > 0:
                ...
                self.detalle += f"{item.nombre:<30} {item.cantidad:<10} {self._format_currency(item.precio):<10} {self._format_currency(subtotal):<10}\n"
    ...
    def crear_pdf(self):
        ...
        pdf.cell(0, 10, "Dirección: Calle Increible", ln=True, align='L')
        ...
        for item in self.pedido.menus:
            if item.cantidad > 0:
                ...
                pdf.cell(35, 10, self._format_currency(item.precio), border=1)
                pdf.cell(30, 10, self._format_currency(subtotal), border=1)
                pdf.ln()
        pdf.set_font("Arial", 'B', 12)
        pdf.cell(120, 10, "Subtotal:", 0, 0, 'R')
        pdf.cell(30, 10, self._format_currency(self.subtotal), ln=True, align='R')
        pdf.cell(120, 10, "IVA (19%):", 0, 0, 'R')
        pdf.cell(30, 10, self._format_currency(self.iva), ln=True, align='R')
        pdf.cell(120, 10, "Total:", 0, 0, 'R')
        pdf.cell(30, 10, self._format_currency(self.total), ln=True, align='R')
